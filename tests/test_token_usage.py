...
async def test_token_accumulation_accuracy_single_player():
    ...
    player = MockXGP("alice", "test_alice", {}, game_config, token_usage_per_move={"input_tokens": 10, "output_tokens": 5})
...
async def test_token_accumulation_accuracy_multiple_players():
    ...
    alice = MockXGP("alice", "test_alice", {}, game_config,
                    token_usage_per_move={"input_tokens": 10, "output_tokens": 5})
    bob = MockXGP("bob", "test_bob", {}, game_config, token_usage_per_move={"input_tokens": 20, "output_tokens": 8})
...
async def test_game_iteration_reset():
    ...
    game_config = FAKE_GAME_CONFIG.copy()
    player = MockXGP("alice", "test_alice", {}, game_config, token_usage_per_move={"input_tokens": 15, "output_tokens": 10})
    locals = build_locals([player], game_config)
    judge = Judge("gpt2")
    globals = build_globals(judge)
    xrt = XegaRuntime([player], locals, globals)
...
async def test_zero_token_usage():
    ...
    game_config = FAKE_GAME_CONFIG.copy()
    player = MockXGP("alice", "test_alice", {}, game_config, token_usage_per_move={"input_tokens": 0, "output_tokens": 0})
    locals = build_locals([player], game_config)
    judge = Judge("gpt2")
    globals = build_globals(judge)
    xrt = XegaRuntime([player], locals, globals)
